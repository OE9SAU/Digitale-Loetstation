...
void setup(){
  ...
  if(!digitalRead(PIN_ROT_PUSH)){schirmSetup();}      // Bei gedruecktem Taster zur Einstellung der Standardwerte
    ...
    lcd.print("SolderUnit V1.5");                     // Begruessungstext 1. Zeile
...
void loop() {
  static unsigned long letzteTemperaturAbfrage = 0;  // Variable für das Intervall der Temperaturabfrage
  static unsigned long letzteRegelung = 0;           // Variable für das Intervall der Temperaturregelung
  
  // Temperaturabfrage alle 250 ms durchführen
  if (millis() - letzteTemperaturAbfrage > 250) {
    abfrageTemperatur();  // Temperaturmessung durchführen
    letzteTemperaturAbfrage = millis();  // Zeitstempel setzen
  }
  
  // Regelkreis alle 1000 ms (1 Sekunde) aufrufen, um die Heizung zu steuern
  if (millis() - letzteRegelung > 1000) {
    reglerTemperatur();  // PID-Regelung aktivieren
    letzteRegelung = millis();  // Zeitstempel für die nächste Regelung setzen
  }

  // Aktualisieren der Temperaturanzeige auf dem Display
  anzeigeTemperatur();  // Temperaturwerte auf dem Display anzeigen

  // Soll-Temperatur ggf. ändern
  aendereTemperatur();  // Setzt neue Soll-Temperatur, wenn nötig

  // Bewegt den Cursor basierend auf der Drehschalterposition
  bewegeCursor();  // Ändert den Cursor entsprechend der Benutzereingabe

  // Setzt ggf. die Soll-Temperatur auf eine der gespeicherten Set-Temperaturen
  waehleSettemperatur();  // Soll-Temperatur ändern, wenn eine gespeicherte Temperatur ausgewählt wird

  // Prüft, ob der Standby-Modus aktiviert wurde
  standbyAbfrage();  // Steuert den Standby-Modus (Lötkolben in der Halterung)

  // LOGO LED flackert zur Anzeige
  logoflackerled();  // Steuert die Flacker-LED für das Logo
}
...
int abfrageTemperatur() {
  static int letzterADC = -1;                         // Letzter gemessener ADC-Wert
  static unsigned long letzteÄnderung = 0;            // Letzter Zeitpunkt einer Änderung
  static byte fehlerZähler = 0;                       // Zähler für fehlerhafte/frozen Zustände

  OCR1B = 0;                                           // Heizung deaktivieren für Messung
  delay(WARTEZEIT);                                   // Zeit für Tiefpassfilter

  // Mehrfachmessung mit Mittelwertbildung zur Glättung
  long summe = 0;
  const byte messungen = 5;
  for(byte i = 0; i < messungen; i++) {
    summe += analogRead(PIN_TEMP);
    delay(2);                                         // Kleine Pause zwischen Messungen
  }
  int tempval = summe / messungen;

  // Extremwerte sofortiger Not-Aus
  if(tempval > 1000 || tempval < 10) {                // Offener Sensor? Kurzschluss?
    notAus();
  }

  // Überwachen auf eingefrorene Werte, nur wenn wirklich geheizt wird
  if(tempval != letzterADC) {
    letzterADC = tempval;
    letzteÄnderung = millis();
    fehlerZähler = 0;                                 // Reset bei Veränderung
  } else {
    if(millis() - letzteÄnderung > 500 && pwm > 200) { // Keine Änderung bei aktiver Heizung
      fehlerZähler++;
      if(fehlerZähler > 4) {
        notAus();                                     // Messwert bleibt verdächtig lange konstant → Fehler
      }
    }
  }

  OCR1B = pwm;                                        // Heizung wieder auf vorherigen Zustand setzen

  // Temperatur aus ADC-Wert berechnen
